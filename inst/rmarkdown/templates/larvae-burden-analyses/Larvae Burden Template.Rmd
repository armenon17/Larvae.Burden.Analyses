...
```{r create fish line column }
burdenall_clean <- cbind(burdenall_clean, "Fish_line")
for (i in 1:length(burdenall_clean$Label)) {
...
burdenall_clean <- cbind(burdenall_clean, "mm_strain")
...
```{r anova, echo=FALSE}
res.aov <- aov(FBU ~ Strain_dpi, data = burden_data_clean)
...
baseplot <- ggplot(data=burden_data_clean, aes(x = factor("Strain_dpi" ,levels = graph_order), y="FBU", fill= "mm_strain"))
